function [hout, T, perm, cmap, theGroups] = dendrogram(Z, varargin)
    %% Ben Ireland, June 2026
    % Small modification to MATLAB in-built dendrogram function to allow
    % for better plotting and colouring.
...
function [theGroups, groups, cmap] = getColorGroups(numLeaves, Z, color, threshold)
    ...
    if color
        ...
        if groups > 1 && groups < (numLeaves - 1)
            ...
            cmap = colorTriplets(2:numColors+1,:);
%             cmap = hsv(numColors);
            cmap(end + 1, :) = [0 0 0];
        else
            groups = 1;
